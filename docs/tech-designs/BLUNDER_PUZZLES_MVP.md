...
- **Implementation notes (as built, August 2026)**:
  ...
  - **The service has zero runtime dependencies** — `node:http` + `node:child_process` only, no Fastify, no ts-rest, no `@tactica/*`. That keeps the image to "Node + engine + our `dist`" with no npm install and no monorepo-in-Docker gymnastics. Consequences: `config.ts` hand-rolls its env parsing instead of using `@tactica/errors`, and `log.ts` writes JSON to stdout directly (the shared eslint config also bans `console.log`). The TypeScript is compiled **on the host** and `dist` is copied in, because the package's tsconfig extends a workspace package that wouldn't resolve inside a build stage.
  ...
  - Endpoints are `GET /health` and `POST /evaluate` with **no internal-API-key check** — unlike tactica-analysis/tactica-coach, this service holds no data and no credentials, so the key would only add ceremony. Revisit if it is ever exposed beyond the local network.
