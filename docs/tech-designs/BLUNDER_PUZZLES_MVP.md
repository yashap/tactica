...
- **Implementation notes (as built, August 2026)**:
  ...
  - **`InternalFastifyAppBuilder`** was added to `@tactica/fastify-utils` for this: `FastifyAppBuilder` hard-registers SuperTokens and CORS and requires a `websiteDomain`, none of which an internal service has. The new builder is the same thing minus those two, and M3/M6 should use it too.
